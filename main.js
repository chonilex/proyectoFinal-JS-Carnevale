...
operacionesForm.addEventListener("submit", function (event) {
  ...
  if (monto > saldo) {
    resultadoOperacion.textContent =
      "Lo siento, el monto ingresado no coincide con el del usuario";
  ...
  mensajeResultado += `<br>Saldo disponible en pesos: $${pesosSobrantes.toFixed(
    2
  )}`;
...
let elementoListaPokemon = document.getElementById("pokemon-list");

function obtenerDatosPokemon(pokemonNombre) {
  return fetch(`https://pokeapi.co/api/v2/pokemon/${pokemonNombre}`)
    .then((respuesta) => {
      if (!respuesta.ok) {
    ...
    .catch((error) => {
      console.error(`Error al obtener datos de ${pokemonNombre}: ${error}`);
    });
}

function mostrarInformacionPokemon(pokemonNombre) {
  obtenerDatosPokemon(pokemonNombre).then((datosPokemon) => {
    let elementoPokemon = document.createElement("div");
    elementoPokemon.classList.add("pokemon-card");
    elementoPokemon.innerHTML = `
                ...
                <button onclick="handleCompraClick('${datosPokemon.name}')">Comprar</button>
            `;
    elementoListaPokemon.appendChild(elementoPokemon);
  });
}

function obtenerYMostrarPokemon() {
  let nombresPokemon = ["bulbasaur", "charmander", "squirtle"];
...
function handleCompraClick(pokemonNombre) {
  Toastify({
    text: `¡Compra exitosa de ${pokemonNombre}!`,
    duration: 3000,
    close: true,
  }).showToast();
}
